const commentEventHandler = async (event) => {
    ...
    if (comment.length < 5) {
        alert('Comment must contain at least 5 characters')
    }
    ...
    if(comment) {
        ...
        if (response >= 400) {
            alert('Error occurred while commenting. Please try again!')
        }
    }
};
...
const editCommentHandler = async (event) => {
    ...
    if (response >= 400) {
        alert('Error occurred while editing. Please try again!')
    }
};
...
const deleteCommentHandler = async (event) => {
    ...
    if (response >= 400) {
        alert('Error occurred while deleting this comment. Please try again!')
    }
};
